    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (result && result.meta && result.meta.requestStatus === 'fulfilled') {
          ...
          setError(isArabic ? 'خارج نطاق التغطية' : 'Out of coverage');
        }
      } else {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <Button onClick={handleGetLocation} className="bg-blue-600 hover:bg-blue-700 w-full py-3 sm:py-4 text-base sm:text-lg font-semibold transition-all duration-200 shadow-sm hover:shadow-md">
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                ...
                {suggestions.length > 0 && (
                  <ul className="absolute z-[2000] bg-white border-2 border-gray-200 w-full max-h-48 sm:max-h-72 overflow-auto rounded-lg shadow-lg mt-2">
                    {suggestions.map(s => (
                      <li
                        key={s.place_id}
              ...
              <Button 
                type="submit" 
                className="bg-red-600 hover:bg-red-700 px-4 sm:px-6 py-2.5 sm:py-3 h-10 sm:h-12 flex items-center gap-1.5 sm:gap-2 transition-all duration-200 shadow-sm hover:shadow-md text-sm sm:text-base" 
                disabled={manualLoading}
              >
                {manualLoading ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-spin" />
                    <span className="hidden sm:inline">{isArabic ? 'جاري...' : 'Loading...'}</span>
                  </>
                ) : (
                  <>
